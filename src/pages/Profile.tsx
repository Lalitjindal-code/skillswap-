...
export default function Profile() {
  ...
  const handleDownloadCV = async () => {
    ...
    try {
      ...
      doc.text("SkillSync Verified Resume", 20, 20);
      ...
      doc.text(`Level: ${user.level} (${user.levelTitle})`, 20, 49);
      doc.text(`Member Since: 2024`, 20, 56);

      // Verified Skills Section
      doc.setFontSize(14);
      doc.setTextColor(0, 0, 0);
      doc.text("VERIFIED SKILLS (Blockchain Secured)", 20, 70);
      doc.line(20, 73, 190, 73);

      doc.setFontSize(10);
      let yPos = 85;

      const skillsToPrint = user.verifiedSkills && user.verifiedSkills.length > 0
        ? user.verifiedSkills
        : [{ name: "No verified skills yet", hash: "-" }];

      skillsToPrint.forEach((skill) => {
        doc.setTextColor(40, 40, 40);
        doc.text(`• ${skill.name}`, 20, yPos);

        doc.setTextColor(100, 100, 100);
        doc.setFont("courier", "normal");
        const hash = skill.hash || getSkillHash(skill.name || "");
        doc.text(`  Hash: ${hash}`, 20, yPos + 5);
      ...
      doc.text("Generated by SkillSync Protocol", 20, pageHeight - 20);
      doc.text("Values: Trust. Learn. Share.", 20, pageHeight - 15);

      doc.save(`${user.name.replace(/\s+/g, '_')}_SkillSync_CV.pdf`);
